...
- Native storage smoke testing and restart persistence verification, provider sign-in/sign-out actions, screen consumption of auth state, route protection, visible loading/error/retry UI, broader authenticated API integration, richer API-error handling, authentication screens, and functional recipe screens remain upcoming work. Provider retry is tested through a test consumer but has no real app control yet. Local token removal does not revoke backend tokens. Restoration distinguishes profile 401 responses from transient failures using structured API errors; it does not restore a user when profile lookup fails.
