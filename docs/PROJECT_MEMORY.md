...
- A discriminated authentication-state union and pure reducer now support loading, signedOut, authenticated, and error states. AuthProvider runs restoration on mount and exposes state through useAuth; cleanup prevents late dispatch but does not cancel requests. The root layout wraps the stack in the provider. Four reducer and five provider tests bring the mobile suite to 44 passing tests; TypeScript checking passes. The user verified Expo Go still launches without runtime errors, not full persisted-session restoration.
...
- Native storage smoke testing and restart persistence verification, provider retry and sign-in/sign-out actions, screen consumption of auth state, route protection, visible loading/error/retry UI, broader authenticated API integration, richer API-error handling, authentication screens, and functional recipe screens remain upcoming work. Local token removal does not revoke backend tokens. Restoration distinguishes profile 401 responses from transient failures using structured API errors; it does not restore a user when profile lookup fails.
