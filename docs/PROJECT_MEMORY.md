...
- The session service coordinates login, profile lookup, and token saving in that order, returning the user only after saving succeeds. Local sign-out awaits token removal without a backend request. Six session tests cover success and dependency failures, bringing the mobile suite to 26 passing tests; TypeScript checking passes. UI authentication state and end-to-end authentication are not yet connected.
...
- Native storage smoke testing and restart persistence verification, login/logout UI integration, session restoration, expired-token handling, broader authenticated API integration, richer API-error handling, authentication screens, and functional recipe screens remain upcoming work. Local token removal does not revoke backend tokens. Restoration must distinguish invalid/expired tokens from network/server failures and preserve credentials on transient failures; use structured API errors rather than message matching.
