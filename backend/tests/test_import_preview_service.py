...
from app.types import FailedImportStatus, SaveableImportStatus
...
@pytest.mark.parametrize(
    ...
    [
        (RecipeImportBlockedError("Import blocked."), "blocked"),
        (RecipeImportFailedError("Import failed."), "failed"),
    ],
)
def test_import_preview_service_logs_response_with_import_errors(
    monkeypatch: pytest.MonkeyPatch,
    import_error: RecipeImportError,
    expected_status: FailedImportStatus,
) -> None:
    session = Mock(spec=Session)
    current_user_id = uuid4()
    lookup = Mock(return_value=None)
    ...
    create_log = Mock(return_value=error_import_log)
    monkeypatch.setattr(import_preview_service, "create_import_log", create_log)
    ...
    assert response.import_id == error_import_log.id
    assert response.status == expected_status
    assert response.draft is None
    assert response.parser_used is None
    assert response.warnings == [str(import_error)]
    assert response.next_actions == ["enter_manually", "open_source_url"]
    importer.preview_from_url.assert_awaited_once_with(SOURCE_URL)
    session.commit.assert_not_called()
    session.rollback.assert_not_called()
